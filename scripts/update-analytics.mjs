...
async function main(){let payload;if(env.MOCK_MODE==='1'){payload=await mock()}else{const missing=required.filter(k=>!env[k]);if(missing.length)throw new Error(`Missing secrets: ${missing.join(', ')}`);const googleConfigured=['GSC_CLIENT_ID','GSC_CLIENT_SECRET','GSC_REFRESH_TOKEN'].every(k=>env[k]);const [yandex,google,clarity]=await Promise.all([safe(fetchYandex,{summary:{},timeline:[],sources:[],pages:[],goals:[]}),googleConfigured?safe(fetchGoogle,{summary:{},queries:[]}):Promise.resolve({__error:'Не подключено: Google OAuth',summary:{},queries:[]}),safe(fetchClarity,{summary:{},raw:[]})]);payload={generatedAt:new Date().toISOString(),period:{yandex:'последние 7 полных дней',google:'последние 7 доступных дней',clarity:'последние 72 часа'},yandex,google,clarity,health:{yandex:{ok:!yandex.__error,message:yandex.__error||'OK'},google:{ok:!google.__error,message:google.__error||'OK'},clarity:{ok:!clarity.__error,message:clarity.__error||'OK'}}}}
