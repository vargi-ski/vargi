...
async function fetchGoogleBridge(){let error;for(let attempt=1;attempt<=3;attempt++){try{const endpoint=url(env.GSC_BRIDGE_URL,{key:env.GSC_BRIDGE_KEY,fresh:Date.now()});const response=await json(endpoint,{headers:{'cache-control':'no-cache'}});if(!response.ok)throw new Error(response.error||'Google bridge returned an error');return response.data}catch(current){error=current;console.warn(`Мост Search Console: ${friendlyError(current)}`);if(attempt===3)throw current;console.log(`Мост Search Console недоступен, повтор ${attempt}/2 через 20 секунд`);await wait(20000)}}throw error}
